...
| Upload keystore + passwords | `android/ingo-upload.jks`, `android/keystore.properties`, and a password manager. Play rejected this key on 2026-09-09 (it expected the original developer's `66:20:0F:B9:…:7D:D5`); an upload key reset was filed with `android/ingo-upload-cert.pem` and accepted, so **from 2026-09-11 22:31 UTC the registered upload key is ours**, `76:1B:9D:40:…:4B:D8` | `git status` shows neither; the App signing page shows `76:1B:9D:40…` |
...
| 10–12 | **waiting until 2026-09-11 22:31 UTC** (2026-09-12 01:31 EAT). Play rejected `InGo v1.1.6.aab` on 09-09 because the registered upload certificate was the original developer's; the reset was requested and accepted on 09-10 and Google's email names `76:1B:9D:40:…:4B:D8`, this repo's key, as the new one. **Nothing can be uploaded to the listing before that moment.** After it, upload the same `InGo v1.1.6.aab` unchanged — no rebuild, no version bump |
