...
| Upload keystore + passwords | `android/ingo-upload.jks`, `android/keystore.properties`, and a password manager. **Play rejected this key on 2026-09-09** (expects `66:20:0F:B9:…:7D:D5`, the original developer's; ours is `76:1B:9D:40:…:4B:D8`). Either request an upload key reset in Play Console with `android/ingo-upload-cert.pem`, or obtain the original keystore | `git status` shows neither; Play accepts the upload |
...
| 10–12 | **blocked 2026-09-09** — Play rejected `InGo v1.1.6.aab`: the registered upload certificate is `66:20:0F:B9:…:7D:D5` (the original developer's), ours is `76:1B:9D:40:…:4B:D8`. Request an upload key reset with `android/ingo-upload-cert.pem`, or obtain the original keystore; then upload the same bundle unchanged (a reset) or rebuild signed with the original key |
